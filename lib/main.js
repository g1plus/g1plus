...
// We need to keep an additional copy of the preferences that is stringifyable
// to pass to our user-script. simplePrefs.prefs will fail on JSON.stringify()
// and therefore can't be passed around using worker.port.emit()
var properPrefs = prefs;
...
function onPrefChange(prefName) {
    properPrefs[prefName] = prefs[prefName];
}
...
pageMod.PageMod({
    ...
    contentScriptFile: [data.url('jquery.min.js'), data.url('swfobject.js'), data.url('konami.pack.js'), data.url('augmentpage.js')],
    onAttach: function(worker) {
        properPrefs.dataRoot = data.url('');

        // Immediately emit the preferences to the user-script
        worker.port.emit('main', {prefs: properPrefs});
